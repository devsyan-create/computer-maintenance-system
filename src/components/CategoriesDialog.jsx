...
export function CategoriesDialog({ open, onOpenChange }) {
  ...
  const handleSaveEdit = async () => {
    if (!editingValue.trim() || categories.some((c, i) => c.name === editingValue.trim() && i !== editingIndex)) {
      return
    }
    ...
    try {
      ...
      updated[editingIndex] = { ...category, name: editingValue.trim() }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl" onClose={() => onOpenChange(false)}>
        <DialogHeader>
          <DialogTitle>بەڕێوەبردنی جۆرەکان</DialogTitle>
        </DialogHeader>
      ...
      <ConfirmDialog
        ...
        title="سڕینەوەی جۆر"
        message="دڵنیای لە سڕینەوەی ئەم جۆرە؟ ئەم کردارە ناگەڕێتەوە."
